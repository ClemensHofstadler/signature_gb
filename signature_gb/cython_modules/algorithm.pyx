...
###########################################################################
    cdef bint F5_criterion(Algorithm self, SigPoly p):
        ...
        a,j,b = p._sig.aib()
        fj = self.gens[j-1].lm()._mon
        
        if not self.lm_automaton: return False
                    
        sigma = Sig(a,j,'')
        for k,(i,m) in self.lm_automaton.iter(a):
            mu = self.G[i]._sig
            mu = mu.lrmul(a[:k-len(m)+1],a[k+1:] + fj)
            if sigma > mu: return True
        
        sigma = Sig('',j,b)
        for k,(i,m) in self.lm_automaton.iter(b):
            mu = self.G[i]._sig
            mu = mu.lrmul(fj + b[:k-len(m)+1],b[k+1:])
            if sigma > mu:  return True
